...
logger = logging.getLogger("build-images-parallel-docker-only")
logger.setLevel(logging.INFO)
log_file_handler = logging.FileHandler('build-images-parallel-docker-only.log')
...
def build_image(image_job):
    ...
    command = ["/vagrant/build/scripts/build-images.sh","-t",image_id, "-d"]
...
def main():
    parser = argparse.ArgumentParser(description='build-images-parallel-docker-only')
    parser.add_argument(
        '-t',
        action="store",
        dest="threads",
