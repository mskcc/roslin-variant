...
inputs:
  ...
  mergeInfoWithMaxAC:
    type: ["null", boolean]
    default: False
    doc: If true, when VCF records overlap the info field is taken from the one with the max AC instead of only taking the fields which are identical across the overlapping records.
    inputBinding:
  ...
  genotypemergeoption:
    ...
    doc: Determines how we should merge genotype records for samples shared across the ROD files (UNIQUIFY| PRIORITIZE|UNSORTED|REQUIRE_UNIQUE)
    inputBinding:
  ...
  filteredrecordsmergetype:
    ...
    doc: Determines how we should handle records seen at the same site in the VCF, but with different FILTER fields (KEEP_IF_ANY_UNFILTERED| KEEP_IF_ALL_UNFILTERED| KEEP_UNCONDITIONAL)
    inputBinding:
  ...
  rod_priority_list:
    ...
    doc: A comma-separated string describing the priority ordering for the genotypes as far as which record gets emitted
    inputBinding:
  ...
  filteredAreUncalled:
    type: ["null", boolean]
    default: False
    doc: If true, then filtered VCFs are treated as uncalled, so that filtered set annotations don't appear in the combined VCF
    inputBinding:
  ...
  minimumN:
    ...
    doc: Combine variants and output site only if the variant is present in at least N input files.
    inputBinding:
      prefix: --minimumN 
