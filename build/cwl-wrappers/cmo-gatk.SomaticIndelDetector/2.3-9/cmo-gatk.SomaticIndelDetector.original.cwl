...
inputs:
  ...
  phone_home:
    ...
    doc: What kind of GATK run report should we generate? STANDARD is the default, can be NO_ET so nothing is posted to the run repository. Please see -home-and-how-does-it-affect-me#latest for details. (NO_ET|STANDARD|STDOUT)
    inputBinding:
      prefix: --phone_home 
  ...
  gatk_key:
    ...
    doc: GATK Key file. Required if running with -et NO_ET. Please see -home-and-how-does-it-affect-me#latest for details.
    inputBinding:
      prefix: --gatk_key 
  ...
  interval_padding:
    ...
    doc: Indicates how many basepairs of padding to include around each of the intervals specified with the -L/--intervals argument
    inputBinding:
  ...
  downsampling_type:
    ...
    doc: Type of reads downsampling to employ at a given locus. Reads will be selected randomly to be removed from the pile based on the method described here (NONE|ALL_READS|BY_SAMPLE)
    inputBinding:
  ...
  downsample_to_coverage:
    ...
    doc: Coverage [integer] to downsample to at any given locus; note that downsampled reads are randomly selected from all possible reads at a locus. For non-locus-based traversals (eg., ReadWalkers), this sets the maximum number of reads at each alignment start position.
    inputBinding:
  ...
  use_legacy_downsampler:
    type: ["null", boolean]
    default: False
    doc: Use the legacy downsampling implementation instead of the newer, less-tested implementation
    inputBinding:
  ...
  allow_potentially_misencoded_quality_scores:
    type: ["null", boolean]
    default: False
    doc: Do not fail when encountered base qualities that are too high and seemingly indicate a problem with the base quality encoding of the BAM file
    inputBinding:
  ...
  refseq:
    ...
    doc: Name of RefSeq transcript annotation file. If specified, indels will be annotated with GENOMIC/UTR/INTRON/CODING and with the gene name
    inputBinding:
      prefix: --refseq 
  ...
  filter_expressions:
    ...
    doc: One or more logical expressions. If any of the expressions is TRUE, putative indel will be discarded and nothing will be printed into the output (unless genotyping at the specific position is explicitly requested, see -genotype). Default - T_COV<6||N_COV<4|| T_INDEL_F<0.3||T_INDEL_CF<0.7
    inputBinding:
  ...
  maxNumberOfReads:
    ...
    doc: Maximum number of reads to cache in the window; if number of reads exceeds this number, the window will be skipped and no calls will be made from it
    inputBinding:
