...
du ${outputs_path} -shL
du ${PRISM_BIN_PATH}/tmp/jobstore-${job_store_uuid} -shL
du ${PRISM_BIN_PATH}/tmp/toil-${workflow_id} -shL

echo
echo "without following any symbolic links"
du ${outputs_path} -sh
du ${PRISM_BIN_PATH}/tmp/jobstore-${job_store_uuid} -sh
du ${PRISM_BIN_PATH}/tmp/toil-${workflow_id} -sh
