...
./install-pipeline.sh \
    -p roslin-${ROSLIN_PIPELINE_NAME}-pipeline-v${ROSLIN_PIPELINE_VERSION}.tgz -f
...
# on aws, installed with root priviledge
# give ownership to ubuntu
chown -R ubuntu:ubuntu ${ROSLIN_CORE_ROOT}
chown -R ubuntu:ubuntu ${ROSLIN_PIPELINE_ROOT}
